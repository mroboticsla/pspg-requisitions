...
import { getPublicJobAds } from '@/lib/jobAds';
  ...
  const loadJobs = useCallback(async (pageNum: number, reset: boolean = false) => {
    try {
      ...
      const { data, count } = await getPublicJobAds({
        ...
        pageSize,
        search: searchTerm,
        employment_type: selectedType === 'all' ? undefined : selectedType,
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-8 flex items-center justify-between">
              <p className="text-lg text-gray-700">
                Mostrando <span className="font-bold text-brand-accent text-xl">{jobs.length}</span>{' '}
                de <span className="font-bold text-gray-900">{totalCount}</span> oportunidades
              </p>
            </div>
              ...
              {!loading && hasMore && (
                <div className="text-center pt-8">
                  <button
                    ...
                    className="bg-white border-2 border-brand-accent text-brand-accent px-8 py-3 rounded-lg font-bold text-lg hover:bg-brand-accent hover:text-white transition-all duration-300 shadow-md"
                  >
                    Cargar más ofertas
                  </button>
                </div>
