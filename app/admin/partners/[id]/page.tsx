...
import { ArrowLeft, Save, User, Mail, Phone, Briefcase, Eye, EyeOff, KeyRound, Send, Lock } from 'lucide-react'
...
export default function PartnerFormPage() {
  ...
  const [partnerRole, setPartnerRole] = useState<RoleOption | null>(null)
  ...
  useEffect(() => {
    ...
    const loadPartnerRole = async () => {
      try {
        ...
        const pRole = allRoles.find((r: RoleOption) => r.name === 'partner')
        if (!pRole) {
          showError('Rol de partner no encontrado')
          router.replace('/admin/partners')
          return
        }
        setPartnerRole(pRole)
      } catch (err: any) {
        showError(err.message || 'Error loading partner role')
        router.replace('/admin/partners')
      }
    }

    loadPartnerRole()
  }, [user, canManage, showError, router])
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        ...
        const body = await res.json()
        if (!res.ok) throw new Error(body.error || 'Error loading partner')

        const users = body.data || []
        const foundUser = users.find((u: UserData) => u.id === userId)
        
        if (!foundUser) {
          showError('Asociado no encontrado')
          router.replace('/admin/partners')
          return
        }
        ...
        const emailRes = await fetch('/api/admin/secure', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
          body: JSON.stringify({ action: 'get-user-email', userId: foundUser.id })
        })
        ...
        showError(err.message || 'Error loading partner data')
        router.replace('/admin/partners')
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!partnerRole) {
      showError('Rol de partner no disponible')
      return
    }
    ...
    try {
      ...
      if (isNew) {
        ...
        const body = await res.json()
        if (!res.ok) throw new Error(body.error || 'Error creating partner')

        success('Asociado creado correctamente')
      } else {
        ...
        const body = await res.json()
        if (!res.ok) throw new Error(body.error || 'Error updating partner')
        ...
        success('Asociado actualizado correctamente')
      }
  ...
  if (loading || loadingUser || !partnerRole) {
    return (
      ...
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-700 font-medium text-sm">
            Acceso restringido. Solo administradores pueden gestionar asociados.
          </p>
        </div>
      </div>
    )
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6">
            ...
            >
              ...
              <span>Volver a Asociados</span>
            </button>
            <h1 className="text-3xl font-bold text-gray-900">
              {isNew ? 'Crear Asociado' : 'Editar Asociado'}
            </h1>
            {!isNew && userData && (
                ...
                >
                  {busy ? (
                    ...
                    <>
                      ...
                      {isNew ? 'Crear Asociado' : 'Guardar Cambios'}
                    </>
                  )}
                </button>
              </div>
