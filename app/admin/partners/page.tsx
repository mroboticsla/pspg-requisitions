...
import { Trash2, Plus, UserCheck, UserX, Edit, Briefcase } from 'lucide-react'
...
export default function PartnersPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const usersWithEmails = await Promise.all(
        partnerUsers.map(async (u: UserRow) => {
          try {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(u => {
      ...
      const phone = u.phone || ''
      const email = u.email || ''
      return fullName.includes(s) || phone.includes(s) || email.toLowerCase().includes(s)
    })
  }, [users, search])
  ...
  const handleEdit = (u: UserRow) => {
    router.push(`/admin/partners/${u.id}`)
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      success('Asociado eliminado exitosamente')
  ...
  const toggleUserStatus = async (u: UserRow) => {
    try {
      ...
      success(`Asociado ${newStatus ? 'activado' : 'desactivado'} exitosamente`)
      await refresh()
    } catch (err: any) {
      showError(err.message || String(err))
    } finally {
      ...
      <div className="space-y-6">
        <div className="space-y-4 p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Gestión de Asociados</h1>
              <p className="text-gray-600 mt-1">Administre los usuarios asociados del sistema</p>
            </div>
            <button 
            ...
            <div className="flex gap-3 pb-2">
              <div className="bg-gradient-to-br from-brand-dark to-[#003d66] rounded-lg shadow-md p-5 text-white flex-shrink-0 w-[280px]">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Briefcase className="w-16 h-16 text-gray-200 opacity-40" />
                </div>
            ...
            <div className="rounded-lg shadow-sm overflow-hidden border border-gray-200">
              ...
              <div className="hidden lg:block bg-gradient-to-r from-brand-dark to-[#003d66] text-white px-4 py-2.5 border-b border-gray-300">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-[180px]">
                    <span className="text-xs font-semibold uppercase tracking-wide">Asociado</span>
                  </div>
              ...
              <div>
                ...
                {filteredUsers.map((partner, index) => {
                  const fullName = `${partner.first_name || ''} ${partner.last_name || ''}`.trim() || 'Sin nombre'
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-2 lg:hidden">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-start gap-2 flex-1 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {partner.email && (
                                <p className="text-xs text-gray-600 leading-tight mt-0.5">📧 {partner.email}</p>
                              )}
                              {partner.phone && (
                        ...
                        <div className={`grid gap-2 mt-1 ${isSuper ? 'grid-cols-3' : 'grid-cols-2'}`}>
                          ...
                          <button
                            disabled={busy}
                            onClick={() => toggleUserStatus(partner)}
                          ...
                          >
                            {partner.is_active ? <UserX className="w-3.5 h-3.5" /> : <UserCheck className="w-3.5 h-3.5" />}
                            <span>{partner.is_active ? 'Desactivar' : 'Activar'}</span>
                          </button>

                          {isSuper && (
                            <button
                              disabled={busy}
                              onClick={() => setShowDelete({ open: true, user: partner })}
                      ...
                      <div className="hidden lg:flex lg:items-center lg:gap-4">
                        ...
                        <div className="w-48 flex-shrink-0">
                          <p className="text-xs text-gray-600 truncate" title={partner.email || 'Sin correo'}>
                            {partner.email || <span className="text-gray-400">Sin correo</span>}
                          </p>
                        </div>
        ...
        <ConfirmModal
          ...
          message={`¿Está seguro de que desea eliminar al asociado "${showDelete.user ? `${showDelete.user.first_name || ''} ${showDelete.user.last_name || ''}`.trim() : ''}"? Esta acción no se puede deshacer.`}
